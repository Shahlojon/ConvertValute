...
const updateTime = () => {
    ...
    const date = nowDate.getUTCFullYear() + "." + dateTime(nowDate.getUTCMonth()) + "." + dateTime(nowDate.getUTCDate());
    $('#currency-date').html(date);
    $('#currency-time').html(time);
};
...
const setCurrency = (data, currency) => {
    $(`#${currency}BuyXazina`).html(data.buy_value.toFixed(4));
    $(`#${currency}SellXazina`).html(data.sell_value.toFixed(4));
    $(`#${currency}BuyIntiqol`).html(data.money_transfer_buy_value.toFixed(4));
    $(`#${currency}SellIntiqol`).html(data.money_transfer_trade_value.toFixed(4));
    $(`#${currency}Bmt`).html(data.nbt_value.toFixed(4));
    // prev[currency] = data;
}
